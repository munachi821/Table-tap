  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      let query = supabase
        ...
        .or(
          `status.eq.paid,and(status.eq.completed,created_at.gte.${twentyFourHoursAgo})`,
        )
      ...
      if (data) {
        const orders = data.map((items) => {
          return {
            orderId: items.id,
            tableNumber:
              (items.tables as unknown as { table_name: string } | null)
                ?.table_name ?? "Unknown",
            status: items.status,
            ...
            items: items.order_items.map((item) => {
              return {
                id: item.id,
                name:
                  (item.menu_items as unknown as { name: string } | null)
                    ?.name ?? "Unknown",
      ...
      <div className="p-4 mt-16">
        {filterOrders.length === 0 ? (
          ...
          filterOrders.map((orders) => (
            <div
              key={orders.orderId}
              className="mb-6 bg-white border border-gray-100 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow"
            >
